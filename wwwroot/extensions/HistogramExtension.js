...
class HistogramExtension extends BaseExtension {
  ...
  onToolbarCreated() {
    this._barChartPanel = new HistogramPanel(
      ...
      "Property Histogram",
      { x: 10, y: 10, chartType: "bar" }
    );
    this._pieChartPanel = new HistogramPanel(
      this,
      "dashboard-piechart-panel",
      "Property Histogram",
      { x: 10, y: 420, chartType: "doughnut" }
    );
    ...
    this._pieChartButton.onClick = () => {
      ...
      this._pieChartButton.setState(
        this._pieChartPanel.isVisible()
          ? Autodesk.Viewing.UI.Button.State.ACTIVE
          : Autodesk.Viewing.UI.Button.State.INACTIVE
      );
